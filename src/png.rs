...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 8 { return Err(PngError::IncompleteSlice.into()); }

        let header_bytes: [u8; 8] = bytes[0..8].try_into()?;
        if header_bytes != Self::STANDARD_HEADER { return Err(PngError::InvalidHeader.into()); }

        let mut chunk_bytes: &[u8] = &bytes[8..];
        let mut chunks = Vec::new();
        while !chunk_bytes.is_empty() {
            if chunk_bytes.len() < 12 {
                return Err(PngError::IncompleteChunk.into());
            }

            let length_bytes: [u8; 4] = chunk_bytes[0..4].try_into()?;
            let length = u32::from_be_bytes(length_bytes) as usize;

            let end: usize = 4 + 4 + length + 4;

            if chunk_bytes.len() < end {
                return Err(PngError::IncompleteChunk.into());
            }

            let chunk = Chunk::try_from(chunk_bytes)?;
            chunks.push(chunk);
            chunk_bytes = &chunk_bytes[end..];
        }
...
impl fmt::Display for PngError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::IncompleteSlice => write!(f, "PNG bytes are incomplete"),
            Self::InvalidHeader => write!(f, "PNG header is invalid"),
            Self::IncompleteChunk => write!(f, "PNG contains an incomplete chunk")
        }
    }
}
